...
class QueryEngine:
    ...
    def parse_query(self, query: dict):  # Проверяет, подходит ли документ под условие
        ...
        if "@or" in query:
            # Рекурсивно вызываем функцию для каждого подзапроса внутри "$or", в итоге получаем список функций-проверок
            or_conditions = [self.parse_query(cond) for cond in query["@or"]]
            # Вернет true если хотя бы один подзапрос (cond) вернет true
            return lambda doc: any(cond(doc) for cond in or_conditions)
        if "@and" in query:
            and_conditions = [self.parse_query(cond) for cond in query["@and"]]
            # Вернет true если все подзапросы (cond) вернут true
